...
export default function CitizenDashboard() {
  ...
  const [userEmail, setUserEmail] = useState<string>('citizen@example.com');
      ...
      try {
        ...
        if (token) {
          ...
          if (res.ok) {
            ...
            if (data.citizenProfile?.full_name) {
              setUserName(data.citizenProfile.full_name);
            } else if (data.email) {
          ...
          if (compRes.ok) {
            ...
            const formatted = compData.map((c: any) => ({
              id: c.id,
              title: c.title,
              description: c.description,
              status: c.status === 'VERIFIED' ? 'In Progress' : c.status === 'RESOLVED' ? 'Resolved' : c.status === 'REJECTED' ? 'Rejected' : 'Pending',
              category: c.department?.name || 'Municipal Department',
        ...
        console.error('Could not fetch user/complaints', e);
      }
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 relative">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-2 text-base font-medium">
          Logged in as <span className="font-bold text-[#042B6B]">{userEmail}</span> • Here's your civic engagement overview for today.
        </p>
      </div>
